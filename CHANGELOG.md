...
- Team tmux workers no longer adopt on a live pane shell. `resolveProviderChild`
  ...
  whose `comm` basename matches the routed executable. Pane-alive with no
  match is `block_identity_unproven`, not `adopt`. tmux/ps failures are
  `unknown`, not `alive`. Start markers now store `ps` `lstart` (PID-reuse-
  safe); session name and provider basename live on the heartbeat and survive
