...
*   **Session skill 面（`skills/*`）**：對齊 OMC/OMX 的 session 內 workflow（`autopilot` / `ralph` / `ultrawork` / `search` / `team` / `cancel` / `verify` / `setup` + index `oma-runtime`）。managed launch 會在 `OMA_TASK` 分隔符外附加 `<<<OMA_SKILL_PROTOCOL>>>`；PreInvocation 在 exact_env 綁定成功時注入 skill 提醒。**CLI 負責啟動與帳本；skill 負責 session 內如何做完。**
