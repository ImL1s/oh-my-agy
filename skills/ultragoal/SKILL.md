---
name: ultragoal
description: "OMA durable implement+verify phase — sequential goals with ledger until evidence passes"
argument-hint: "<approved plan path or goal brief>"
---

# ultragoal (OMA / Antigravity)

## Purpose

Durable **implement + verify** phase (OMX `$ultragoal` analogue). Runs after `ralplan` APPROVE and before `code-review`.

Maps to Autopilot active phase `ultragoal` (legacy: `executing`). Tools are Antigravity-native + `oma` CLI — not Claude `Task` / OMX `.omx` paths.
...
- Spec still vague → `deep-interview`
- Plan not approved → `ralplan` (do not implement on REVISE)
- Only parallel independent slices without multi-goal ledger → `ultrawork`
- Full product pipeline orchestration → follow parent `autopilot` skill
...
Optional machine summary for autopilot handoff:
...
| `skills/team/SKILL.md` | Optional multi-worker DAG via `oma team …` |
| `skills/ralph/SKILL.md` | Persistence loop pattern for single-threaded stories |
| `skills/ultrawork/SKILL.md` | Parallel independent slices inside a subgoal |
| `skills/code-review/SKILL.md` | Next phase after ultragoal evidence |

## CLI ledger (outer)

When running under Autopilot, keep the outer session in sync:

```bash
# record implement artifact path
oma autopilot handoff --session <id> --expected-revision <n> \
  --key ultragoal --path .agy/ultragoal/<slug>/handoff.json

# gate advance with phase evidence (kind: ultragoal | executing)
oma autopilot advance --session <id> --expected-revision <n> \
  --evidence <path-to-evidence.json>

# optional process drive for current phase skill inject
oma autopilot drive --session <id> --conversation <cid> --expected-revision <n>

oma autopilot status --session <id>
```

Evidence JSON must prove implement work + verify ran (see `verify` skill). Do not advance on "looks good".
...
8. **Phase handoff** — write handoff summary; `oma autopilot handoff --key ultragoal`; prepare evidence for advance into `code-review`.
9. **Do not self-approve merge** — next phase is `code-review` (merge-readiness), then `ultraqa`.
...
- [ ] Handoff path recorded for Autopilot when session-bound

## Stop / return conditions

- Hard blocker (credentials, product decision) → report; do not invent requirements
- Same verify failure 3× with no new diagnosis → escalate; consider `return-ralplan` via parent autopilot
...
- Writing completion into OMA state as `verified: true` yourself — outer CLI gates own that
...
- [ ] Ready for `skills/code-review/SKILL.md` + `oma autopilot review`
