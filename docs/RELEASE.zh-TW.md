...
OMA `0.5.2` 將 GitHub Release tarball 加 `SHA256SUMS` 視為唯一可安裝的發佈渠道。本儲存庫**目前不**發佈到 npmjs.org 或 GitHub Packages。`@iml1s/oh-my-agy` 是 tarball 內的 package identity，不能證明 registry 條目存在。
