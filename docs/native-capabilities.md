...
- `native probe --live` is the only command that opts into a live probe. The v1
  ...
  model-bearing canaries use a fixed 32-process cumulative lineage budget
  (release-host samples peaked at 19) so Antigravity can start the user's
  configured MCP services, while passive help/version inspection remains at 8;
  ...
  zombie PIDs are excluded. Unproven PID-1 baseline-delta processes are not
  adopted; only parent-tree or process-group evidence can own a PID, so
  unrelated post-baseline load cannot create a false processCountOverflow.
