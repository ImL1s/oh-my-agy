...
這套凍結 grammar 只是既有 fallback adapter 的相容性 metadata，不是 provider 或 native-capability gate。fresh-session plugin-discovery canary 同樣容忍已觀察到的 agy 1.1.6 尾隨雙換行，並 canonical 化儲存的 evidence 位元組。
