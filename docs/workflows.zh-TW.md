...
- **Prompt 是 `--print` 的即時值。** 尾隨 prompt 會讓 agy 1.1.5 把後續 flag 吞進 prompt 文字。
...
fresh-session plugin-discovery canary 同樣固定，並容忍 agy 1.1.5 的尾隨雙換行，canonical 化儲存的 evidence 位元組。
